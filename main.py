...
def geturl(userid,start):
    ...
    for work in works:
        ImageName = re.sub(r'[/\\:*?"<>|]*', '', work["title"])
        if (str(requests.get('https://pixiv.cat/'+str(work["id"])+'.jpg')) == '<Response [404]>'):
            imgID = 1
            while True:
                if (str(requests.get('https://pixiv.cat/'+str(work["id"])+'-'+str(imgID)+'.jpg')) == '<Response [404]>'):
                    break
                f = open("image/"+ImageName+"-"+str(imgID)+".png",'wb')
                response = requests.get('https://pixiv.cat/'+str(work["id"])+'-'+str(imgID)+'.jpg')
                
                print('https://pixiv.cat/'+str(work["id"])+'-'+str(imgID)+'.jpg')

                f.write(response.content)
                f.close()
                imgs+=1
                print(str(imgs)+". "+ImageName)
                imgID+=1
        else:
            f = open("image/"+ImageName+".png",'wb')
            response = requests.get('https://pixiv.cat/'+str(work["id"])+'.jpg')

            print('https://pixiv.cat/'+str(work["id"])+'.jpg')
